...
# 3.21 Fri
* 147.预测处理识别意识神经相关物  CreateAMind  https://mp.weixin.qq.com/s/PD6bhK1KuKUaPDc75OlSbQ \
  Predictive processing as a systematic basis for identifying the neural correlates of consciousness \
  预测处理作为识别意识神经相关物的系统化基础
* 148.[DeepRetrieval] 用DeepSeek-R1-Zero的思路教会模型怎么用搜索引擎找文本  关于NLP那些你不知道的事  https://mp.weixin.qq.com/s/0m4yyvYt8RumjkfysirVAQ \
...
* 149.树搜索也存在「过思考」与「欠思考」？腾讯AI Lab与厦大联合提出高效树搜索框架  机器之心  https://mp.weixin.qq.com/s/TZVzF5YWOduMFLFybs47Gw \
  高效树搜索框架——Fetch \
  Don't Get Lost in the Trees: Streamlining LLM Reasoning by Overcoming Tree Search Exploration Pitfalls \
  尝试解决LLM树搜索存在的「过思考-欠思考」双重困境
* 150.对话周光：自动驾驶实现AGI，RoadAGI比L5更快 | GTC 2025  量子位  https://mp.weixin.qq.com/s/krR9dqehKS8IRB6aNZNByg \
  元戎启行的RoadAGI
* 151.最近爆火的MCP(Model Context Protocol)，读这一篇就够了  Datawhale  https://mp.weixin.qq.com/s/ULxokHOn4zVOgiLHf9DQUA 

# 3.22 Sat
* 152.认知地图结构向量代数，GC-VSA统一框架整合空间和符号信息的能力  CreateAMind  https://mp.weixin.qq.com/s/AUWpiXELqQ20YfDzJEtUuQ \
  A Grid Cell-Inspired Structured Vector Algebra forCognitive Maps \
  一种受网格细胞启发的认知地图结构化向量代数 \
  连续吸引子网络（CANs ontinuous attractor networks）???
* 153.具有无限数量专家的专家混合模型，超网络增强规则学习的多样性和准确性  CreateAMind  https://mp.weixin.qq.com/s/G0A5Q6ZPO97nrIP3yJKH-g \
  HyperLogic: Enhancing Diversity and Accuracy inRule Learning with HyperNets \
  HyperLogic：利用超网络增强规则学习中的多样性和准确性 \
  探索在神经网络架构中整合“如果-那么”逻辑规则是一个有趣的研究领域。这种整合将规则学习任务无缝转化为使用反向传播和随机梯度下降的神经网络训练。从一个训练良好的稀疏且浅层的神经网络中，我们可以通过逻辑规则的语言来解释每一层和每一个神经元，并直接提取出一个全局的解释性规则集。然而，确保可解释性可能会对神经网络的灵活性、深度和宽度施加限制。在本文中，我们提出了HyperLogic：**一种利用超网络生成主网络权重的灵活方法**。HyperLogic可以与现有的可微分规则学习方法相结合，生成多样化的规则集，每个规则集都能捕捉数据中的异质性模式。这为增加模型灵活性和保持可解释性提供了一种简单而有效的方法。我们通过检查两种类型的正则化项——稀疏性和多样性正则化——下的近似误差和泛化能力，对HyperLogic的理论优势进行了分析。在真实数据上的实验表明，我们的方法能够学习到更多样化、准确且简洁的规则。我们的代码已公开在[https://github.com/YangYang-624/HyperLogic](https://github.com/YangYang-624/HyperLogic)。 \
  ???超网络具体是如何生成主网络的
* 154.预测编码推理与学习的理论框架  CreateAMind  https://mp.weixin.qq.com/s/DptypUJfzGy9-se8rswUaA \
...
* 155.揭秘DeepSeek R1-Zero训练方式，GRPO还有极简改进方案  机器之心  https://mp.weixin.qq.com/s/SBGO_1JXnI9CGcLL8eANBA \
  Understanding R1-Zero-Like Training: A Critical Perspective \
  https://github.com/sail-sg/understand-r1-zero \
   Dr. GRPO
* 156.强化学习也涌现？自监督RL扩展到1000层网络，机器人任务提升50倍  机器之心  https://mp.weixin.qq.com/s/jharVbDajhmWmSfpTjstWg \
...
* 157.ICLR 2025 Spotlight｜让机器人实现「自主进化」，蚂蚁数科、清华提出具身协同框架 BodyGen  机器之心  https://mp.weixin.qq.com/s/6exmJvisas2ZYgWB9YadJg \
...
* 158.Idea撞车何恺明「分形生成模型」！速度领先10倍，性能更强  新智元  https://mp.weixin.qq.com/s/sF3HYIog0bCpDBJ8Gk_KmQ \
...
* 159.英伟达开源自适应多模态「世界生成」模型！开启机器人、自动驾驶训练革命  新智元  https://mp.weixin.qq.com/s/BqYNHHuvGNUBzPjekWaoGQ \
...
* 160.自我模拟和预测：机器实现自指又迈进了一步  集智俱乐部  https://mp.weixin.qq.com/s/lQxY37nSgMMXpct0rXAkkQ \
...
# 3.23 Sun
* 161.意识复杂性的剖析：理论与反思（3万字）  CreateAMind  https://mp.weixin.qq.com/s/jAstoKXDTMI4jesJ40IOzg \
...
* 162.「注意力实际上是对数的」？七年前的Transformer还有新发现，Karpathy点赞  机器之心  https://mp.weixin.qq.com/s/iDWcMYhTpqs-4hqsPGmVaA 
* 163.田渊栋和Sergey Levine参与开发新型RL算法，能通过多轮训练让智能体学会协作推理  机器之心  https://mp.weixin.qq.com/s/sgNsSM0Vp0dIcUp08lWSFQ \
  SWEET-RL: Training Multi-Turn LLM Agents on Collaborative Reasoning Tasks \
  https://github.com/facebookresearch/sweet_rl \
  目前尚不清楚哪种多轮 RL 算法最有效，能够充分利用 LLM 的推理能力来训练通用、有能力和目标导向的智能体。 \
...
* 164.用科幻建立AI行为准则？DeepMind提出首个此类基准并构建了机器人宪法  机器之心  https://mp.weixin.qq.com/s/8RRKmyXXUORUuKyMfTH4-g \
  SciFi-Benchmark: How Would AI-Powered Robots Behave in Science Fiction Literature?
* 165.真正的LLM Agent  Datawhale  https://mp.weixin.qq.com/s/DoCgIacZ5LQIK31op8PCaw 
* 166.院士领衔万字长文，全面系统梳理多模态LLM对齐算法  量子位  https://mp.weixin.qq.com/s/8djZcfyl3v0N1eZRQBk0Yg \
  Aligning Multimodal LLM with Human Preference: A Survey \
  https://github.com/BradyFU/Awesome-Multimodal-Large-Language-Models/tree/Alignment
* 167.谷歌对齐大模型与人脑信号！语言理解生成机制高度一致，成果登Nature子刊  量子位  https://mp.weixin.qq.com/s/wDJltfLGRHv3TyfcI4jdPA  \
  离散扩散模型，称为GIDD，无需任何监督微调（SFT）或强化学习（RL）能够直接从预训练中学会自我纠错 \
  GIDD核心主要有3大特征： \
...
* 168.中科院领衔万字长文，全面系统梳理多模态LLM对齐算法  量子位  https://mp.weixin.qq.com/s/8djZcfyl3v0N1eZRQBk0Yg \
  Aligning Multimodal LLM with Human Preference: A Survey
* 169.意识复杂性的剖析：理论与反思（3万字）  CreateAMind  https://mp.weixin.qq.com/s/jAstoKXDTMI4jesJ40IOzg \
  Unpacking the complexities of consciousness: Theories and reflections

# 3.24 Mon
* 170.蒙特卡洛树MCTS和LLM相遇了会发生什么？  关于NLP那些你不知道的事  https://mp.weixin.qq.com/s/fqfytrdw92lo24eLOdsrlQ
* 171.多模态思维链推理：全面综述  专知  https://mp.weixin.qq.com/s/rkOFkbD3SRiVsbc-otWbDA \
  Multimodal Chain-of-Thought Reasoning:A Comprehensive Survey
* 172.CVPR 2025 | Qwen让AI「看见」三维世界，SeeGround实现零样本开放词汇3D视觉定位  机器之心  https://mp.weixin.qq.com/s/LakS8zqiA5XunmEQykKCDw \
...
* 173.为什么明明很准，奖励模型就是不work？新研究：准确度 is not all you need  机器之心  https://mp.weixin.qq.com/s/mNE_S1qpx5-HOLZGpWPXrQ \
  What Makes a Reward Model a Good Teacher? An Optimization Perspective \
  如何衡量 RM 的质量呢？普林斯顿大学一个研究团队发现，如果仅用准确度来衡量 RM 的质量，可能无法完全体现一个奖励模型作为有效教师的特性。为此，他们选择了从优化角度来研究这个问题。 \
  ...
  这些结果说明：如果在设计奖励模型时仅基于准确度或不考虑具体的语言模型，那么就会遭遇一些根本性的限制。总体而言，除了准确度之外，奖励模型还需要**诱导出足够的方差**才能实现有效优化。
* 174.意识计算模型-最小体验现象  CreateAMind  https://mp.weixin.qq.com/s/uEUy2q7DMxW82IPcJqjRKg \
...
* 175.单卡3090纯视觉玩转MineCraft，发ICLR 2025 Oral！  Datawhale  https://mp.weixin.qq.com/s/ivXGNWKWw8SHj30hjNOyjw \
  Open-World Reinforcement Learning over Long Short-Term Imagination \
  https://qiwang067.github.io/ls-imagine \
  LS-Imagine
* 176.万字长文！从AI Agent到Agent工作流，一文详细了解代理工作流(Agentic Workflows)  AINLPer  https://mp.weixin.qq.com/s/wGZ7G4e-Rch_EqirFt0Ybg 

# 3.25 Tue
* 177.构建像人一样学习思考的机器（4万字）  CreateAMind  https://mp.weixin.qq.com/s/EXuplw_kSRsbE533sOWHGw \
...
* 178.推理延展到真实物理世界，英伟达Cosmos-Reason1：8B具身推理表现超过OpenAI ο1  机器之心  https://mp.weixin.qq.com/s/4q94N0ulQv7HXGWH7Pd3vg \
  Cosmos-Reason1: From Physical Common Sense To Embodied Reasoning \
  https://github.com/nvidia-cosmos/cosmos-reason1 \
  物理 AI 是指使用运动技能理解现实世界并与之进行交互的模型，它们通常封装在机器人或自动驾驶汽车等自主机器中。
* 179.基于大语言模型的智能体优化研究综述  专知  https://mp.weixin.qq.com/s/7UQE1JOBPRgrum-627Z1vw \
  A Survey on the Optimization of Large Language Model-based Agents \
  https://github.com/YoungDubbyDu/LLM-Agent-Optimization 
* 180.大模型在简单物理现象理解任务上的认知困境：联想能力将是AGI之路上的下一个关键？  PaperWeekly  https://mp.weixin.qq.com/s/n8sbF3NIjVOSyyICfoZZ9w \
...
# 3.26 Wed
* 181.推理系统的任意性：一阶逻辑推理，概率推理，因果推理，牛顿力学，空间推理，不可证伪的推理  CreateAMind  https://mp.weixin.qq.com/s/s-ybTW_BzFHZbKjbYGh5mw \
  From machine learning to machine reasoning
* 182.130页pdf《长上下文语言建模综合研究》综述  专知  https://mp.weixin.qq.com/s/AThq-DlgdcXNQwa5S-bxQQ \
  长上下文语言模型（LCLMs）的构建、训练与部署、评估与分析 \
  A Comprehensive Survey on Long Context Language Modeling

# 3.27 Thur
* 183.合并符号与神经策略学习的强化学习框架  CreateAMind  https://mp.weixin.qq.com/s/CBUuaJzBQ4mneE2e_iEASw \
  ...
  神经符号强化学习框架  BlendRL \
  BlendRL=反应模块（神经网络）+推理模块（逻辑策略）
* 184.腾讯混元、英伟达都发混合架构模型，Mamba-Transformer要崛起吗？  机器之心  https://mp.weixin.qq.com/s/KWCE3mxZeWchpDM9YmxpnQ \
  Mamba-Transformer相对于纯mamba和纯transformer的优势是啥？
* 185.AI Agents全栈技术框架综述与未来！  Datawhale  https://mp.weixin.qq.com/s/fwmilVyIZ5LEfYMtm1t1jA 
* 186.7B模型搞定AI视频通话，阿里最新开源炸场，看听说写全模态打通，开发者企业免费商用  量子位  https://mp.weixin.qq.com/s/VkJP89bpgZaxaKz-1wav9A \
  Qwen2.5-Omni-7B
* 187.长视频理解新突破！Mamba混合架构让显存消耗腰斩，处理10万视频token不费力  量子位  https://mp.weixin.qq.com/s/eGBF_jJqmpwNT6D-t0BK5Q \
  VAMBA: Understanding Hour-Long Videos with Hybrid Mamba-Transformers \
  https://tiger-ai-lab.github.io/Vamba/ \
  https://github.com/TIGER-AI-Lab/Vamba \
  https://huggingface.co/TIGER-Lab/Vamba-Qwen2-VL-7B
* 188.《基于大语言模型的数学推理与优化研究综述》  专知  https://mp.weixin.qq.com/s/y9vVTlVoTKkkN9ua9WxZWg \
  A Survey on Mathematical Reasoning and Optimization with Large Language Models
* 189.看行业DeepSeekR1模型如何构建及减少推理大模型过度思考  关于NLP那些你不知道的事  https://mp.weixin.qq.com/s/iPLJQjfFKOdeHgLmDAEjYw \
  PRMs是什么？它们如何训练、数据如何标注，以及在预测时如何使用？ \
  MCTS在LLM场景中是如何实现的？ \
  PRMs和MCTS是完全独立的技术，还是相辅相成的？
* 190.清华稀疏Attention，无需训练加速一切模型！  机器之心  https://mp.weixin.qq.com/s/uDpCOeITwsQRjC4V7LRNHw \
  SpargeAttn: Accurate Sparse Attention Accelerating Any Model Inference \
  https://github.com/thu-ml/SpargeAttn \
  无需训练可直接使用的稀疏 Attention（SpargeAttn）可用来加速任意模型。实现了 4-7 倍相比于 FlashAttention 的推理加速，且在语言，视频、图像生成等大模型上均保持了端到端的精度表现。
* 191.（**值得看看**）超越功能主义：意识科学的数学结构性转向  集智俱乐部  https://mp.weixin.qq.com/s/m9xzf0u3ujmPouHdRwa51Q \
  “我们不该再问‘这是否是一种意识体验’，而应探究‘这个体验具有怎样的数学结构’。” 
  
# 3.28 Fri
* 192.意识科学之战：超越伪科学与伪意识  神经现实  https://mp.weixin.qq.com/s/lUt0-FkhRsIG0hC-5-0dBA \
  Consciousnessor pseudo-consciousness? A clash of two paradigms \
  What makes a theory of consciousness unscientific?
* 193.Anthropic首次切开Claude大脑，「AI黑箱」彻底破解？心算诡异思考过程曝光  新智元  https://mp.weixin.qq.com/s/jNe9smcObXyoMES6dgv4ig \
  怒发两篇Paper！Anthropic | 解密大语言模型的“内心活动”，且具备独有概念空间！  AINLPer  https://mp.weixin.qq.com/s/ASFazU-uD9d9vKyoRq-XUg \
  Tracing the thoughts of a large language model \
  博客地址：https://www.anthropic.com/research/tracing-thoughts-language-model

# 3.29 Sat
* 194.《大语言模型智能体：方法、应用与挑战综述》  专知  https://mp.weixin.qq.com/s/knMa4vXVs_1iYMCNFLNrSw \
  Large Language Model Agent: A Survey on Methodology, Applications and Challenges \
  https://github.com/luo-junyu/Awesome-Agent-Papers
* 195.启示AGI之路 : 脑机制自下而上梳理 上部 5万字  CreateAMind  https://mp.weixin.qq.com/s/osFD2g2g-8ipZjxUuiuctw \
  AGI面临的挑战 : 脑机制自底向顶梳理 下部 5万字  CreateAMind  https://mp.weixin.qq.com/s/mlNlZcWJ4vXRLOeNXb_B_g \
...
* 196.这篇GPU学习笔记，详细整理了其工作原理、编程模型和架构设计  吃果冻不吐果冻皮  https://mp.weixin.qq.com/s/YVhVpOQf6ih0v3aAFkXsmQ

# 3.30 Sun
* 197.OpenManus 任务拆分和动态规划实现机制  关于NLP那些你不知道的事  https://mp.weixin.qq.com/s/hYIpgd-_bKQb9Mr9sAAJNQ \
  OpenManus 采用多层架构设计实现任务拆分和动态规划，主要包括以下核心组件： \
    ...
    Manus：执行具体任务的代理 \
  整体工作流程： \
    1.使用 PlanningFlow 进行任务规划和拆解
    2.通过 LLM 动态生成执行计划 \
    3.使用 Manus 代理执行具体任务 \
    4.支持任务状态追踪和动态调整 \
  OpenManus 的任务拆分和动态规划机制具有以下优势： \
    1.灵活性：可以处理各种类型的任务，从简单的网页浏览到复杂的数据分析 \
    2.可扩展性：易于添加新的执行器和工具，扩展系统功能 \
    3.鲁棒性：完善的状态管理和错误处理机制，确保任务执行的稳定性 \
    4.智能性：利用 LLM 的理解能力动态规划任务，自适应不同任务特点 \
    5.可追踪性：详细记录每个步骤的执行过程和结果，便于调试和优化
* 198.元认知粒子、心理行动与代理感  CreateAMind  https://mp.weixin.qq.com/s/dIVpkqYUygiq5ltLvslOng \
  ...
  结论：我们提出了一个在贝叶斯力学和自由能原理框架内，基于统计物理学的元认知定义。元认知，被理解为形成关于信念的信念的能力，在这种形式主义中找到了自然的数学表述。我们进一步区分了元认知的不同类型和程度，例如强元认知与弱元认知、主动元认知与被动元认知。通过这种方式，我们为代理感（与代理本身的在场不同）找到了一个数学对应物，并论证了如此定义的元认知是这种感觉的必要条件。我们将[11]中认知粒子的分类扩展到包括更接近我们自身的粒子，从而为复杂认知生物体中起作用的物理和信息动态的进一步实证和数学研究开辟了方向。
* 199.机器人大脑架构技术  CreateAMind  https://mp.weixin.qq.com/s/3Ih585YXn0vmf2cptDF6_Q \
  高维向量计算 \
  机器人学的超维计算导论 \
  认知地图结构向量代数，GC-VSA统一框架整合空间和符号信息的能力 \
  认知地图的规范计算模型实现
* 200.业界突破多模态泛化推理能力，OPPO研究院&港科广提出OThink-MR1技术  量子位  https://mp.weixin.qq.com/s/gWGn02IQpO-K75sshqeUfg \
  OThink-MR1: Stimulating multimodal generalized reasoning capabilities via dynamic reinforcementlearning \
  来自OPPO研究院和港科广的科研人员提出了一项新技术——OThink-MR1，将强化学习扩展到多模态语言模型，帮助其更好地应对各种复杂任务和新场景。 \
  OThink-MR1是一个基于动态强化学习的框架和模型，支持微调多模态语言模型。其核心“招式”有两个：一个是动态KL散度策略（GRPO-D），另一个是精心设计的奖励模型。二者相互配合，让模型的学习效率和推理能力大幅提升。
* 201.模型调优无需标注数据！将Llama 3.3 70B直接提升到GPT-4o水平  机器之心  https://mp.weixin.qq.com/s/WfJtfEN1gHg5NvgCYwefsw \
  ...
  总结：垂类微调
* 202.卷积网络又双叒叕行了？OverLoCK:一种仿生的卷积神经网络视觉基础模型  机器之心  https://mp.weixin.qq.com/s/SLOpxechpAl6mXjkgqscrg \
...
* 203.《高效推理的大模型研究综述：语言、多模态与前沿探索》  专知  https://mp.weixin.qq.com/s/VsRIB-yiKjoY2yp-kAmUNw \
  A Survey of Effcient Reasoning for Large Reasoning Models: Language, Multimodality, and Beyond 
  
# 3.31 Mon
* 204.超维计算/向量符号架构中维度的“祝福”：推理与学习的展望  CreateAMind  https://mp.weixin.qq.com/s/jZ5O_qsbjUe2Njt7RfwUCw \
...
* 205.200美金，人人可手搓QwQ，清华、蚂蚁开源极速RL框架AReaL-boba  机器之心  https://mp.weixin.qq.com/s/Cx8QHv2TVl-0mIJzKT7BDA \
...
* 206.视频生成中的物理认知演进探究：一项综述  专知  https://mp.weixin.qq.com/s/A7uvcrM47XPTLwYXZEw-3g \
  Exploring the Evolution of Physics Cognition inVideo Generation: A Survey \
  从认知科学视角梳理了视频生成中物理认知的演进过程，并提出三层分类体系：1）面向生成的基础图式感知；2）基于物理知识的被动认知生成；3）面向世界模拟的主动认知
* 207.意识理论列表  CreateAMind  https://mp.weixin.qq.com/s/kvBh-jod3akoLRW1MbdLDQ \
  意识理论 概述 \
  意识的数学模型   （4万字） \
  意识复杂性的剖析：理论与反思（3万字） \
  意识计算模型-最小体验现象 \
  意识的贝叶斯理论：寻找最小统一模型的综述2021
